...
import { Leaf, Recycle, Bot, AlertTriangle, Droplets, Waves } from 'lucide-react';
...
const BentoGrid = () => {
  ...
  const [hazardModal, setHazardModal] = useState(null);
  ...
  return (
    ...
    <div className="flex flex-col xl:flex-row xl:flex-nowrap xl:items-center xl:justify-center xl:overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden items-center justify-center gap-5 w-full max-w-[1400px] mx-auto mb-6 px-2 sm:px-4">
      ...
      {/* CARD 4: HAZARD REPORTING */}
      <div className="w-full max-w-[420px] xl:max-w-none xl:w-[240px] xl:shrink-0 min-h-[300px] sm:min-h-[320px] bg-gradient-to-b from-[#1a4032] to-[#0f291e] rounded-[1.5rem] p-5 sm:p-6 flex flex-col justify-between items-center text-center relative overflow-hidden group hover:-translate-y-1 transition-transform duration-500 text-white">
          ...
          <div className="relative z-10 flex flex-col items-center flex-1">
            <div className="w-12 h-12 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center mb-4">
                <AlertTriangle size={22} className="text-[#C3F53C]" />
            </div>
            ...
            <h3 className="text-2xl font-bold leading-tight mb-4">
                Report Flooding or Drainage Blockage
            </h3>
          </div>
          ...
          <div className="relative z-10 w-full space-y-2 mt-2">
            <button
              type="button"
              onClick={() => setHazardModal('drainage_blockage')}
              className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-bold transition-colors"
            >
              <Droplets size={14} /> Report Drainage Blockage
            </button>
            <button
              type="button"
              onClick={() => setHazardModal('flood')}
              className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-[#C3F53C] hover:bg-[#b2e32b] text-[#1a4032] text-xs font-bold transition-colors"
            >
              <Waves size={14} /> Report Flood
            </button>
          </div>
      </div>
    ...
    <ReportHazardModal
        isOpen={!!hazardModal}
        reportType={hazardModal || 'drainage_blockage'}
