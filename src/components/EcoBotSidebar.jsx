...
const EcoBotSidebar = ({ isOpen, onClose }) => {
  ...
  const handleActionClick = (action) => {
    ...
    // Slight delay to allow sidebar to close smoothly before navigating
    setTimeout(() => {
  ...
  // --- CUSTOM MARKDOWN RENDERER (THE FIX) ---
  const MarkdownComponents = {
    a: ({ href, children }) => {
        // 1. Check for our special Hash Action links (#action:...)
        if (href && href.startsWith('#action:')) {
            const action = href.replace('#action:', ''); // Remove prefix
            ...
            return (
                <button 
                    ...
                    className="
                        ...
                        no-underline block mb-2
                    "
                >
                    <Icon size={16} className="text-[#C3F53C]" />
                    {/* Clean up text (Remove 'BUTTON:' if AI added it) */}
                    <span>{String(children).replace('BUTTON: ', '')}</span>
                </button>
            );
        }
        
        // 2. Regular Links (WhatsApp, Instagram)
        return (
            <a 
                href={href} 
                target="_blank" 
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 font-bold text-green-700 hover:text-green-800 hover:underline decoration-2 underline-offset-2 transition-colors"
            >
                {children}
    ...
    <div className="fixed inset-0 z-[100] flex justify-end font-sans">
      ...
      <div className={`relative z-10 w-full md:w-[400px] bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="bg-slate-50 border-b border-slate-100 p-4 text-center">
            <p className="text-slate-500 text-sm font-medium">Ask me about recycling or <br/><span className="text-[#1a4032] font-bold">KanemWaste Services</span></p>
        </div>
