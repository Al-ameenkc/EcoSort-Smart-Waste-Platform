...
const AdminDashboard = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setFloodReports(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
    return () => unsubscribe();
  }, []);
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex-1 lg:h-screen overflow-y-auto p-4 md:p-6 lg:p-8 lg:pt-24">
        ...
        {activeTab !== 'optimizer' && activeTab !== 'logistics' && activeTab !== 'hazards' && (
        ...
        {activeTab === 'logistics' && (
             <LogisticsDashboard floodReports={floodReports} />
        )}

        {activeTab === 'hazards' && (
            <>
                <div className="mb-6">
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Hazard Reports</h1>
                    <p className="text-slate-500 mt-1">
                        Total reports: <span className="font-bold text-slate-900">{floodReports.length}</span>
                        {' · '}
                        Open: <span className="font-bold text-slate-900">{floodReports.filter(r => r.status !== 'Resolved').length}</span>
                    </p>
                </div>
                <FloodReportsPanel reports={floodReports} />
            </>
        )}
